...
class GodelEncoder:
    ...
    def encode_alpha(self, alpha_reversed):
        """
        α′ already reversed: [state, sym1, sym2, ...] — reversed order.
        ψ_x(α′) = γ_q(d1)*nq^{-1} + Σ γ_s(d_k)*ns^{-(k+1)}
        """
        state = alpha_reversed[0]
        tail = alpha_reversed[1:]
        
        val = self.gamma_q[state] / self.nq 
        for k, sym in enumerate(tail, start=1):
            val += self.gamma_s[sym] / ((self.ns ** k) * self.nq)
        
        return val

    def encode_beta(self, beta):
        """
        β = [sym1, sym2, sym3, ...] — normal order.
        ψ_x(β) = Σ γ_s(d_k)*ns^{-k}
        """
        val = 0
        for k, sym in enumerate(beta, start=1):
            val += self.gamma_s[sym] * (self.ns ** -k)
        return val
...
# # α = reversed(left of dot), β = right of dot
# alpha = s[:dot_index]
# beta = s[dot_index+1:]
